...
export function createV2HelpDialog({ root = document } = {}) {
  ...
  const panel = createElement("div", { className: "project-dialog-panel" }, [
    ...
    createElement("section", {}, [
      ...
      createElement("p", { textContent: "Notes in one Pattern may meet end to start but cannot overlap at any pitch. Build chords and other simultaneous parts on separate Tracks. A conflicting draw, move, transpose, resize, duplicate, paste or import is rejected as a whole, leaving the notes, selection and undo history unchanged." }),
      createElement("p", { textContent: "Click the Pattern name in the Piano Roll title, or focus it and press Enter, to rename that Pattern. Drag the rest of the title bar to move the Piano Roll. Space remains the playback control while the title is focused." }),
      createElement("p", { textContent: "Right-click a Playlist Instrument tile for Rename Instrument, Duplicate Instrument and New Pattern. Rename changes its shared Instrument and Track name. Duplicate creates an independent Track below it with the same Klinto Chip sound, a unique name and fresh identity; Mixer settings, Effects and clips are not copied. The new Instrument becomes the Playlist destination and can be undone. At eight Tracks, duplication is disabled." }),
